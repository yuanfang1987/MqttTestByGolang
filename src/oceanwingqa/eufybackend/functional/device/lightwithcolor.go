...
func (light *LightWithColor) buildWhiteLightData() *lightEvent.LampLightLevelCtlMessage {
	brightness := uint32(utils.GenerateRandNumber(10, 100))
	color := uint32(utils.GenerateRandNumber(10, 100))
...
func (light *LightWithColor) buildColorLightData() *lightEvent.LampLightRgbCtlMessage {
	...
	if expRGB, ok := v.(*rgbInfo); ok {
		...
		red = uint32(utils.GenerateRandNumber(0, 255))
		green = uint32(utils.GenerateRandNumber(0, 255))
		blue = uint32(utils.GenerateRandNumber(0, 255))
		rrggbb := &rgbInfo{
			red:   red,
			green: green,
			blue:  blue,
...
func (light *LightWithColor) buildStreamLightData() *light1013.StreamLight {
	brightness := int32(utils.GenerateRandNumber(20, 100))
	// 储存当前数据，用于后续心跳判断
	light.bright = uint32(brightness)
	// 速度， 固定 1 秒， 不好吧
	speed := int32(utils.GenerateRandNumber(1, 3))
...
func (light *LightWithColor) buildRGBData(p string) *light1013.RGBMessage {
	...
	switch p {
	case "A":
		seed = utils.GenerateRandNumber(0, 12)
	case "B":
		seed = utils.GenerateRandNumber(12, 24)
	case "C":
		seed = utils.GenerateRandNumber(24, 36)
	case "D":
		seed = utils.GenerateRandNumber(36, 51)
	}
...
func (light *LightWithColor) unMarshalHeartBeatMessage(payload []byte) {
	...
	contents := []string{light.ProdCode, light.DevKEY, light.testcase, utils.GetCurrentTime()}
...
func (light *LightWithColor) unMarshalServerMessage(payload []byte) {
		...
		if syncAlert != nil {
			...
			if records != nil {
				...
				for i, rec := range records {
					...
					weekinfo := utils.ConvertToWeekDay(alertmsg.GetWeekInfo())
	...
	if away != nil {
		...
		log.Infof("----WeekInfo: %s", utils.ConvertToWeekDay(leaveMsg.GetWeekInfo()))
	}
...
func getRGBData() {
	...
	content, err := utils.ReadFileContent("rgb.txt")
	if err != nil {
