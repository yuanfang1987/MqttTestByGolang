...
func (light *Light) setLightBrightAndColor() *lightT1012.ServerMessage {
	...
	seed := utils.GenerateRandNumber(0, 10)
...
func (light *Light) unMarshalServerMsg(incomingPayload []byte) {
		...
		if alarm != nil {
			...
			for i, v := range alarmRecordData {
				...
				if alarmMsg != nil {
					...
					weekinfo := utils.ConvertToWeekDay(alarmMsg.GetWeekInfo())
	...
	if awayMod != nil {
		...
		if leaveMsg != nil {
			...
			log.Infof("------离家模式, WeekInfo: %s", utils.ConvertToWeekDay(leaveMsg.GetWeekInfo()))
