...
import (
	...
	"oceanwingqa/eufybackend/performance/device"
	"os"
	"os/signal"
	"runtime"
	"strings"
...
func main() {
	...
	eufyDevList, err := utils.ReadFileContent(*filePath)
	if err != nil {
	...
	for i := start; i < end; i++ {
		...
		go func() {
			...
			str := strings.Split(v, ",")
			// new a eufy device.
			eufyDevice := device.NewEufyDevice(str[1], str[1], "oceanwingtest", *broker, str[0], str[1], *needca)
